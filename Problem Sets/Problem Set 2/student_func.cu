...
{ 
  ...
  // NOTE: If a thread's absolute 2D position is within the image, but some of
                       ...
                       int numCols)
{
  ...
  outputImageRGBA[idx] = outputPixel;
}
                                ...
                                const size_t filterWidth)
{
  ...
  //original
  size_t p_size = sizeof(unsigned char) * numRowsImage * numColsImage;
  checkCudaErrors(cudaMalloc(&d_red,   p_size));
                        ...
                        const int filterWidth)
{
  ...
  const dim3 gridSize( numRows/n+1, numCols/n+1, 1);
